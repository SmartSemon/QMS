...
public class MD5Utils
{
	...
	public static boolean validCiphertext(String inputString, String ciphertext) throws Exception
	{
		...
		System.arraycopy(ciphertextInDb, 0, salt, 0, SALT_LENGTH);
		// 创建消息摘要对象
		...
		// 声明一个保存数据库中口令消息摘要的变量
		byte[] digestInDb = new byte[ciphertextInDb.length - SALT_LENGTH];

		// 取得数据库中口令的消息摘要
		System.arraycopy(ciphertextInDb, SALT_LENGTH, digestInDb, 0, digestInDb.length);
